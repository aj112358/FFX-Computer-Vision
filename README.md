...
The second goal was completed through a data analysis. The IDE of choice for the analysis was a Jupyter Notebook. After importing the raw data and performing data cleaning procedures, new features were created and the subsequent majority of time was spent within the exploratory data analysis phase.
...
Finally, we investigated the suspected alternating pattern. Recall from the first histogram above, we hypothesized that the lightning strikes occur in an alternating fashion, first with ~6s between them, then ~11.25s. Indeed, we created a function to determine if this were true and the answer is a resounding 'yes'! *(See detailed analysis for more information)*
